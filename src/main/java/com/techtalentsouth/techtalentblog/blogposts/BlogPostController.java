...
//	@RequestMapping(path="/", method = {RequestMethod.GET, RequestMethod.POST})
	...
	public String index(Model model) {
		//since we're using the @Controller
		//the string we are returning is actually
		//a reference to a HTML template page.
	...
	public String updateExistingPost(@PathVariable Long id, BlogPost blogPost, Model model) {
		...
		if(postBox.isPresent()) {
			...
			blogPostRepository.save(actualPost); //updates the existing blogPost
			//it's an update rather than creating a new database
			//entry because actualPost has the "id" field set to the primary
			//key of an exist database record.
